...
Train_length = 30
Test_length = 7
WINDOW = 48  # 默认以一天共48个数据点作为整体输入LSTM
...
class London_11_14_set_test(Dataset):
    ...
    def __init__(self, flod=0, k_flod=20, train_l=Train_length, label_l=Test_length, test_days=10,
                 test_continuous=1, size=SIZE, times=TIMES, k_flod_test_list=None, window=WINDOW):
        ...
        self.data_test = []
        other = London_11_14_random_select(train_l=self.train_l, test_l=self.label_l, size=self.size, window=window)
        # 取出测试集
...
def createDataSet(k_flod=10, train_l=Train_length, label_l=Test_length, test_days=10,
                  test_continuous=1, size=SIZE, times=TIMES, ev_key=1, window=WINDOW):
    """
        :param k_flod:k折交叉验证
        :param train_l：X天数
        :param label_l：y天数
    ...
    for flod in range(k_flod):
        set2 = London_11_14_set_test(flod=flod, k_flod=k_flod, train_l=train_l, label_l=label_l, test_days=test_days,
                                     ...
                                     k_flod_test_list=k_flod_test_list, window=window)
